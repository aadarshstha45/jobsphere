  ...
  return subItems && subItems.length > 0 ? (
      ...
      >
        ...
        >
          <HStack justify={{ base: "center", md: "flex-start" }}>
            {item.icon && <Icon as={item.icon} boxSize={6} />}
            <Text
        ...
        {subItems.map((subItem, index) => (
          ...
          >
            <Flex as={NavLink} to={subItem.to} key={index} {...attributes}>
              {subItem.icon && <Icon as={subItem.icon} boxSize={6} />}
              <Text fontSize={{ md: "16px" }}>{subItem.title}</Text>
            </Flex>
          </Tooltip>
        ))}
      </Flex>
    ...
    >
      <Flex as={NavLink} to={item.to} {...attributes}>
        {item.icon && <Icon as={item.icon} boxSize={6} />}
        <Text fontSize={{ md: "16px" }}>{item.title}</Text>
      </Flex>
    </Tooltip>
  );
};
